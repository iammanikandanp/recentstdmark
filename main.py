...
if submit != None:
    o="""select regno,ugmark from register where (name='%s' and dob='%s') or (email='%s' and dob='%s') or (name='%s' and password='%s') or (email='%s' and password='%s')"""% (name,pwd,name,pwd,name,pwd,name,pwd)
    cur.execute(o)
    hot=cur.fetchone()
    if hot != None:
        print("""<script>
           alert("login successfully");
           location.href="stdr.py?poiuytrewq=%s&asdfghjkl=%s"
           </script>"""%(hot[0],hot[1]))
    elif submit != None:
        p = """select regno,id from adminreg  where (email="%s" and pass="%s") or (name="%s" and pass="%s") """ % (name,pwd,name,pwd)
        cur.execute(p)
        rec = cur.fetchone()
        if rec != None:
            print("""<script>
            alert("log in successfully");
                location.href='admindash.py?poiuytrewq=%s&lkjhgfdsa=%s'
                </script>
                """ %( rec[0],rec[1]))
        else:
            print("""<script>
