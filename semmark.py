...
cur.execute(p)
hot = cur.fetchall()
y="""select total from webinter order by regno"""
cur.execute(y)
now=cur.fetchall()
c="""select total from javainter order by regno"""
cur.execute(c)
wow=cur.fetchall()
d="""select total from semint order by regno"""
cur.execute(d)
cow=cur.fetchall()
...
if submit != None:
    for (i,k,h,g,x,y,z) in itertools.zip_longest(hot,cow,now,wow, ml, wb, java):
        ...
        if "A" in z:
            ...
            gk=int(z)
        javas=s+gk
        jl = """insert into semmark (regno,name,mli,wdi,javai,eml,ewd,ejava,ml,wd,java) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')on duplicate key update name=values(name),mli=values(mli),wdi=values(wdi),javai=values(javai),eml=values(eml),ewd=values(ewd),ejava=values(ejava),ml=values(ml),wd=values(wd),java=values(java)"""%(i[0],i[1],u,j,s,x,y,z,mls,wds,javas)
        cur.execute(jl)
        con.commit()
        print("""<script>
      ...
      document.addEventListener("DOMContentLoaded",function(){
        ...
        inputs.forEach(input =>{
            input.addEventListener("input",function(){
                ...
                if (!((values >= 0 && values <=75) || values === 'A')){
                    alert("Marks upto 75. Invalid characters are not allowed ")
                    input.value=""
                }else{
