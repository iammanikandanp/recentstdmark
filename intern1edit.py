...
cur = con.cursor()
o = """select regno,name from register order by regno"""
cur.execute(o)
hot = cur.fetchall()
print("""<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="hg">
    <h1 class="ty">1 INTERNAL MARKSHEET [edit]</h1>
    </div>
    <div class="container">
    ...
    <table class="table table-hover table-striped table-bordered kl">
        ...
        """)
p = """select rego,name,ml,wd,java from internal order by rego"""
cur.execute(p)
hot = cur.fetchall()
for i in hot:
    print("""
    ...
    for (i, y, z, k) in itertools.zip_longest(hot, ml, wb, java):
        if "A" in (y and z and k):
            ...
            if "A" in k:
                ...
                ijava = int(k) / 10
            gh = """insert into internal (rego,name,ml,wd,java,mli,wdi,javai) values ('%s','%s','%s','%s','%s','%s','%s','%s')on duplicate key update name=values(name),ml=values(ml),wd=values(wd),java=values(java),mli=values(mli),wdi=values(wdi),javai=values(javai)""" % (
