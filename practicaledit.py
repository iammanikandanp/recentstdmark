...
p = """select regno,name,regular,observation,viva,records,external from practical order by regno"""
cur.execute(p)
hot = cur.fetchall()
for i in hot:
    print("""
        <tr>
         <td>%s</td>
           <td>%s</td>
           <td><input type="text" name="reg" class="regular" value="%s" required></td>
           <td><input type="text" name="obs" class="obser" value="%s" required ></td>
           <td><input type="text" name="viva" class="viva" value="%s" required></td>
           <td><input type="text" name="recs" class="record" value="%s" required></td>
           <td><input type="text" name="pra" class="pra" value="%s" required ></td>
...
if submit != None:
    for (i, y, z, k, j, l) in itertools.zip_longest(hot, reg, obs, viva, recs, pra):
        ...
        if "A" in l:
            ...
            e = int(l)
        b = a + b + c + d + e
        jk = """insert into practical (regno,name,regular,observation,viva,records,external,total) values ('%s','%s','%s','%s','%s','%s','%s','%s')on duplicate key update name=values(name),regular=values(regular),observation=values(observation),viva=values(viva),records=values(records),external=values(external),total=values(total)""" % (
        i[0], i[1], y, z, k, j, l, b)
        cur.execute(jk)
        con.commit()
        print("""
