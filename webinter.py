...
u = """select wb from intercal order by regno"""
cur.execute(u)
now = cur.fetchall()

o = """select wdi from model order by regno"""
cur.execute(o)
wow = cur.fetchall()
for x in hot:
    print("""
...
if sumbit != None:
    for (i, x, y, z, k) in itertools.zip_longest(hot, now, wow, sem, att):
        ...
        t = iny + moe + sem + att
        n = round(t)
        tr = """INSERT INTO webinter(regno,name,internal,model,sem,attendance, total) values ('%s','%s','%s','%s','%s','%s','%s')on duplicate key update name=values(name),internal=values(internal),model=values(model),sem=values(sem),attendance=values(attendance),total=values(total)""" % (
        i[0], i[1], d, e, z, k, n)
        cur.execute(tr)
        con.commit()
        print("""
